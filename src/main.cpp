...
uint256 hashGenesisBlock("0xe16dda19e84c4cd1d2947f46e21872b0ada473d958587f0fd114225053b618e4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gabencoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gabencoin Signed Message:\n";
...
{
    // Gabencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gabencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Gabencoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2/21/2014 Steam  75% Off Dawn Of Fantasy Kingdom Wars - All Hail Thee";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Gabencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GabencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GabencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GabencoinMiner(CWallet *pwallet)
{
    printf("GabencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gabencoin-miner");
    ...
    try { loop {
        ...
        printf("Running GabencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GabencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GabencoinMiner, pwallet));
}
